...
from appo.components import mpi_tools, core, Buffer, EpochLogger
from appo.components.utils import get_flat_params_from

from appo.models import ConstraintActorCritic
...
class PolicyGradient:
    ...
    def __init__(
        ...
        **kwargs,  # use to log parameters from child classes
        ...
        self.use_kl_early_stopping = (
            use_kl_early_stopping  # importance for ppo_base methods
        )
        ...
        # reserving memory for lagrange multiplier
        self.lagrangian_multiplier = None
        self.lambda_range_projection = lambda x: x

    def update_params_from_local(self, local_params):
        params = {
            k: v
            for k, v in local_params.items()
            # if k != "self" and k != "kwargs" and k != "params"
            if k not in ["self", "kwargs", "params"]
        }
        params = deepcopy(params)
        self.params.update(**params)

    def load_actor_pi(self, state_dict_filepath):
        self.ac.pi.load_state_dict(torch.load(state_dict_filepath))
    ...
    def _init_learning_rate_scheduler(self):
        ...
        if self.use_linear_lr_decay:
            ...
            def lm(epoch):
                return 1 - epoch / self.epochs  # linear anneal
    ...
    def learn(self):
        ...
        if mpi_tools.proc_id() == 0:
            fpath = osp.join(self.logger.log_dir, "ac.pkl")
            torch.save(self.ac, fpath)
    ...
    def roll_out(self):
        ...
        o, ep_ret, ep_costs, ep_len = self.env.reset(), 0.0, 0.0, 0

        if (
            self.use_reward_penalty
            and hasattr(self, "lagrangian_multiplier")
            and hasattr(self, "lambda_range_projection")
        ):
            # include reward penalty parameter in reward calculation: r' = r - c
            assert self.lagrangian_multiplier is not None
            penalty_param = self.lambda_range_projection(self.lagrangian_multiplier)
        else:
            penalty_param = 0

        for t in range(self.local_steps_per_epoch):
            a, v, cv, logp = self.ac.step(torch.as_tensor(o, dtype=torch.float32))
            next_o, r, d, info = self.env.step(a)
            c = info.get("cost", 0.0)
            if "v3" in self.env_id:
            ...
            self.buf.store(obs=o, act=a, rew=r, val=v, logp=logp, cost=c, cost_val=cv)
            if self.use_cost_value_function:
                self.logger.store(**{"Values/V": v, "Values/C": cv})
            else:
    ...
    def update_policy_net(self, data) -> None:
        ...
        pi_l_old, pi_info_old = self.compute_loss_pi(
            data,
        )
        ...
        for i in range(self.train_pi_iterations):
            ...
            if self.use_max_grad_norm:  # apply L2 norm
                torch.nn.utils.clip_grad_norm_(
                    self.ac.pi.parameters(), self.max_grad_norm
                )
